...
open class DefaultLogImpl() : ILogger {

    private val pTrss = CopyOnWriteArrayList<IPrinter>()
    ...
    fun removePrinter(printer: IPrinter) {
        pTrss.remove(printer)
    }

    override fun print(level: LogLevel, tag: String?, vararg objs: Any) {
        pTrss.forEach {
            it.dispatchLog(level, tag, *objs)
        }
    }
}